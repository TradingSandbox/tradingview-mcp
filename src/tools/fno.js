...
export function registerFnoTools(server) {
  server.tool(
    ...
    'Get the option chain for an underlying from TradingView\'s scanner — strikes with call/put bid/ask/last/volume, greeks (delta, gamma, theta, vega, rho) and implied volatility (iv_pct). Auto-detects the underlying from the current chart symbol, defaults to the NEAREST upcoming expiration, and returns an ATM-centered window of strikes. Use options_expirations first to pick an expiration. Note: IV is a percentage; TradingView does not expose open interest.',
    {
      ...
      strikes: z.number().int().optional().describe('Number of strikes centered on ATM to return. Default 20. Pass 0 for all (capped at 500).'),
  ...
  server.tool(
    'futures_curve',
    'Get the futures term structure (all listed contract months) for a root from TradingView\'s scanner — each contract with expiration, last price, % change and days-to-expiry, plus front/back contracts and contango/backwardation detection. Auto-detects the root from the current chart symbol (continuous like "NYMEX:CL1!" and dated like "NYMEX:CLF2027" both resolve to root "NYMEX:CL").',
    {
